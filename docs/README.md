...
### 🎨 Графический интерфейс (Desktop GUI) - РЕКОМЕНДУЕТСЯ
Для удобства пользователей доступен графический интерфейс на PyQt6:
...
**Функции GUI:**
- ✅ **Вкладка "⚙️ Конфигурация"** - Удобная форма для введения всех параметров (без редактирования .env)
- ✅ **Вкладка "▶️ Запуск"** - Кнопки старт/стоп, реал-тайм логирование, статистика
- ✅ **Вкладка "📊 История"** - Таблица всех отправленных откликов
- ✅ **Dry-run режим** - Тестирование без реальных откликов
- ✅ **Сохранение профилей** - Сохранить конфигурацию и легко переключаться

### 📋 Командная строка (CLI)
Также продолжает поддерживаться запуск через командную строку:
...
## Создание exe файла (для продажи/распространения)
...
Exe файл будет создан в папке `dist/HH_Auto_Apply/HH_Auto_Apply.exe`

### Расширенный способ
```bash
pyinstaller --onefile --windowed --add-data "hh_auto_apply:hh_auto_apply" --add-data ".env:." --hidden-import=PyQt6 run_gui.py -n "HH_Auto_Apply"
```
